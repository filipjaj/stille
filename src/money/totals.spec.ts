...
describe('calculateTotals', () => {
  it('ganger enhetspris med antall', () => {
    const totals = calculateTotals([{ unitGross: 12500, quantity: 3, vatRate: 25 }], noShipping)
  ...
  it('holder gross = net + vat for hver linje', () => {
    const totals = calculateTotals([{ unitGross: 3333, quantity: 7, vatRate: 15 }], noShipping)
  ...
  it('legger frakt til totalen med sin egen sats', () => {
    const totals = calculateTotals([{ unitGross: 20000, quantity: 1, vatRate: 0 }], {
      gross: 12500,
      vatRate: 25,
    })
  ...
  it('utelater frakt fra MVA-grupperingen når frakten er gratis', () => {
    const totals = calculateTotals([{ unitGross: 20000, quantity: 1, vatRate: 15 }], noShipping)
